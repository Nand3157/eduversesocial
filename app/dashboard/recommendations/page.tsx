...
export default function RecommendationsPage() {
  ...
  const [dismissed, setDismissed] = useState<string[]>(() => {
    ...
    try { const raw = localStorage.getItem("eduverse:dismissed-recs"); return raw ? JSON.parse(raw) as string[] : []; } catch { return []; }
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {toast && <div className="rounded-xl border border-success/25 bg-success/10 px-4 py-3 text-sm text-success flex items-center justify-between"><span>{toast}</span><button onClick={() => setToast(null)} className="ml-2 text-success/70 hover:text-success"><X className="h-4 w-4" /></button></div>}
      {dismissedCount > 0 && (
        <div className="flex items-center gap-2 text-xs text-mutedText">
          <span>{dismissedCount} dismissed</span>
          <button onClick={() => { setDismissed([]); setToast("Restored dismissed recommendations."); setTimeout(()=>setToast(null),2500); }} className="inline-flex items-center gap-1 text-primary hover:underline"><RotateCcw className="h-3 w-3" /> Restore all</button>
        </div>
      )}
      {loading ? <Card><CardContent className="p-6 text-sm text-mutedText">Loading live recommendations…</CardContent></Card> : recommendations.length ? <>
        <motion.div {...fadeUp}>
          <Card className="border-primary/25 bg-accent-soft"><CardContent className="flex flex-col justify-between gap-5 p-6 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><Badge variant="primary">Live recommendation · featured</Badge><h2 className="mt-3 font-heading text-2xl font-medium tracking-tight text-ink">{recommendations[0][0]}</h2><p className="mt-1 text-xs font-medium text-primary">{recommendations[0][1]}</p><p className="mt-2 max-w-2xl text-sm leading-6 text-mutedText">{recommendations[0][2]}</p></div><div className="flex shrink-0 flex-wrap gap-2"><Button className="bg-ink text-background hover:bg-ink/90" onClick={() => handleSchedule(recommendations[0])}><Send className="h-4 w-4" />Schedule this</Button><Button variant="secondary" onClick={() => handleWhy(recommendations[0])}><Info className="h-4 w-4" />Why this?</Button><Button variant="ghost" size="icon" aria-label="Dismiss" onClick={() => handleDismiss(recommendations[0][0])}><X className="h-4 w-4" /></Button></div></CardContent></Card>
        </motion.div>
        <motion.div className="grid gap-5 lg:grid-cols-3" variants={staggerContainer} initial="hidden" animate="show">{recommendations.map(([title, timing, detail]) => <TiltCard key={title} variants={staggerItem} tiltLimit={8} scale={1.02} className="h-full"><Card className="h-full flex flex-col"><CardHeader><div className="flex items-center justify-between"><CalendarClock className="h-5 w-5 text-primary" /><div className="flex gap-1"><Badge variant="success">Ready</Badge><button onClick={() => handleDismiss(title)} aria-label={`Dismiss ${title}`} className="rounded-full p-1 text-mutedText hover:bg-surface hover:text-ink"><X className="h-3.5 w-3.5" /></button></div></div><CardTitle className="pt-3 line-clamp-2">{title}</CardTitle><CardDescription>{timing}</CardDescription></CardHeader><CardContent className="flex flex-1 flex-col"><p className="text-sm leading-6 text-mutedText line-clamp-3">{detail}</p><div className="mt-5 flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={() => handleSchedule([title, timing, detail])}><Send className="h-3.5 w-3.5" />Schedule</Button><Button size="sm" variant="ghost" onClick={() => handleWhy([title, timing, detail])}><Eye className="h-3.5 w-3.5" />Why</Button></div></CardContent></Card></TiltCard>)}</motion.div>
      </> : <Card><CardContent className="p-6"><div className="rounded-xl border border-dashed border-borderSoft bg-surface/50 p-6 text-sm leading-relaxed text-mutedText">No recommendation is being invented. Connect Meta and load enough real post and engagement history before EduVerse suggests a next move.{dismissedCount>0 && " You dismissed some — restore above to see them."}</div></CardContent></Card>}
          ...
          {whyRec && (
            <div className="mt-4 space-y-4">
              ...
              <div className="flex justify-between gap-2 pt-2">
                ...
                <Button variant="primary" onClick={() => { if (whyRec) handleSchedule(whyRec); setWhyRec(null); }}><Sparkles className="h-4 w-4" /> Schedule this</Button>
              </div>
            </div>
          )}
        </ModalContent>
