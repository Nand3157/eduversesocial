...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Exchange short-lived user token (1-2h) for long-lived (30d). Page tokens
    // derived from a long-lived user token are themselves long-lived for 30 days,
    // so this eliminates frequent re-verify in Dev Mode.
