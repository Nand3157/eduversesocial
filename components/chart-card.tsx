...
function ChartState({ loading, message = "Connect Meta to load live analytics." }: { loading: boolean; message?: string }) {
  return <div className="grid h-[220px] place-items-center rounded-xl border border-dashed border-borderSoft bg-surface/50 px-6 text-center text-xs leading-relaxed text-mutedText">{loading ? "Loading live Meta analytics…" : message}</div>;
}
