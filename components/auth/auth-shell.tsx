...
export function AuthShell({ children, title, description }: { children: React.ReactNode; title: string; description: string }) {
  return (
    <main className="relative isolate grid min-h-screen place-items-center overflow-hidden bg-background p-4 sm:p-8">
      <AuthBackground />
      ...
      <div className="relative z-10 grid w-full max-w-5xl items-center gap-10 lg:grid-cols-[1fr_420px]">
        <div className="hidden max-w-lg lg:block">
          <p className="inline-flex items-center gap-2 rounded-full border border-primary/25 bg-accent-soft px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-primary"><Sparkles className="h-3.5 w-3.5" /> EduVerse workspace</p>
          <h2 className="mt-6 text-balance font-display text-6xl font-medium leading-[0.98] tracking-tight text-ink">Make every signal a <em className="italic text-primary">lesson.</em></h2>
        ...
        <section className="w-full rounded-[28px] border border-borderSoft/80 bg-card/85 p-6 shadow-glass backdrop-blur-xl sm:p-8">
          <Link aria-label="EduVerse home" className="inline-flex items-center gap-2.5" href="/">
            <span className="grid h-10 w-10 place-items-center rounded-full bg-primary text-background"><Sparkles className="h-5 w-5" /></span>
