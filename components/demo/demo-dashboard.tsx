    ...
    <div className="space-y-6">
      ...
      <Card className="border-amber-500/25 bg-amber-500/10">
        <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/privacy">
              <Button size="sm" variant="secondary">How data is handled</Button>
            </Link>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          <Badge variant="primary" className="gap-1.5">
            <Sparkles className="h-3 w-3" /> Simulated Graph API
          </Badge>
          <Badge variant="success" className="gap-1.5">
            <BadgeCheck className="h-3 w-3" /> No tokens stored
          </Badge>
        </div>
      </div>
          ...
          <CardContent>
            <div className="grid gap-5 lg:grid-cols-[220px_1fr] lg:items-center">
              <div className="h-[210px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={snap.platformBreakdown} dataKey="value" innerRadius={54} outerRadius={82} paddingAngle={4} cx="50%" cy="50%">
              ...
              <div className="grid gap-3">
                {snap.platformBreakdown.map((p, i) => (
                  <div key={p.name} className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <strong className="text-ink">{p.value}%</strong>
                  </div>
                ))}
              </div>
            </div>
          ...
          <CardContent className="space-y-5">
            {snap.recommendations.map(([title, timing, detail]) => (
              <div key={title} className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Link href="/signup">
                    <Button className="bg-ink text-background hover:bg-ink/90">
                      <Sparkles className="h-4 w-4" /> Sign up to generate your own
                    </Button>
                  </Link>
                  <Link href="/privacy">
                    <Button variant="secondary">How recommendations use data</Button>
                  </Link>
                </div>
              </div>
            ))}
            ...
            <div className="space-y-4">
              {snap.memoryItems.map((item, index) => (
                <div key={item} className="relative flex gap-3">
                  <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full border border-success/30 bg-success/10">
                    <Check className="h-3.5 w-3.5 text-success" />
                  </span>
                  {index < snap.memoryItems.length - 1 && <span className="absolute left-3 top-7 h-8 w-px bg-borderSoft" />}
                  <div>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-borderSoft bg-surface/50 px-4 py-3">
            ...
            <Link href="/signup">
              <Button size="sm" className="bg-primary text-background hover:bg-primary-strong">
                Explore with your data <ArrowRight className="h-3.5 w-3.5" />
              </Button>
            </Link>
          </div>
        </CardContent>
      ...
      <Card className="border-primary/20 bg-accent-soft/30">
        <CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link href="/signup">
            <Button className="bg-ink text-background hover:bg-ink/90">Start free — no OAuth needed to peek</Button>
          </Link>
        </CardContent>
      </Card>
    </div>
  ...
  return (
    <div className="rounded-2xl border border-borderSoft bg-card p-4 shadow-glass">
      ...
      <div className="mt-3 flex items-center justify-between text-[11px] text-mutedText">
        <span className="inline-flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-success animate-pulse" /> No login required</span>
